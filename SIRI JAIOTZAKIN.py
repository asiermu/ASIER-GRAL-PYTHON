...
def kurbenmozketa(mu,beta,eta,gamma):
    ...
    kont=0
    for a in range(100):
        if beta*s[a]-gamma-eta-mu<0:#Asintota dagoenenko  kasua konpondu, ze pythonek grafiko jarrai bezala irudikatzen du.
            i1[a]=(-eta+eta*s[a])/(beta*s[a]-gamma-eta-mu)
            kont+=1
    plt.plot(s[:kont],i1[:kont],color="purple")
    for a in range(100):
        if beta*s[a]-gamma-eta-mu>0:
            i2[a]=(-eta+eta*s[a])/(beta*s[a]-gamma-eta-mu)
...
kurbenmozketa(3,4,3,2)
plt.text(0.1, 0.1, "1", fontsize=10)
plt.text(0.5, 0.33, "2", fontsize=10)
...
plt.arrow(0.2, 0.06, 0, 0.1,
                  head_width = 0.02,
                  width = 0.005)
plt.arrow(0.4, 0.4, 0.1, 0,
                 head_width = 0.02,
                  width = 0.005)
plt.arrow(0.4, 0.4, 0, -0.1,
                  head_width = 0.02,
                  width = 0.005)
...
fase_diagramaemaitza(3,4,3,2,4000000)
show()
egonkortasuna(1,4,3,2,2090)
